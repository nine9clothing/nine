...
const Navbar = ({ showLogo }) => {
  // Initialize user and fullName from localStorage synchronously
  const initialState = (() => {
    ...
    if (persistedUser) {
      try {
        ...
        // console.log('Loaded user from localStorage:', user.id);
      } catch (e) {
    ...
    if (persistedFullName) {
      try {
        ...
        // console.log('Loaded fullName from localStorage:', fullName);
      } catch (e) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (session?.user) {
          ...
          try {
            ...
            // Fetch fullName from Supabase and store in localStorage
            const { data, error } = await supabase
              .from('registered_details')
              .select('full_name')
            ...
            if (error) {
              ...
              console.log('Full name fetched and stored:', fetchedFullName);
            }
          } catch (e) {
    ...
    const refreshInterval = setInterval(async () => {
      try {
        ...
        if (session?.user) {
          ...
          setUser(session.user);
          localStorage.setItem('user', JSON.stringify(session.user));
          // Refresh fullName from Supabase and update localStorage
          const { data, error } = await supabase
            .from('registered_details')
            .select('full_name')
            .eq('email', session.user.email)
          ...
          if (error) {
            ...
            setFullName(fetchedFullName);
            localStorage.setItem('fullName', fetchedFullName);
            console.log('Full name refreshed and stored:', fetchedFullName);
          }
        } else {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      console.log('Auth state changed, user:', session?.user?.id || 'none');
      if (session?.user) {
        setUser(session.user);
        try {
          localStorage.setItem('user', JSON.stringify(session.user));
          // Fetch fullName on auth state change and store in localStorage
          const fetchFullName = async () => {
            ...
            if (error) {
              ...
              setFullName(fetchedFullName);
              localStorage.setItem('fullName', fetchedFullName);
              console.log('Full name fetched on auth change:', fetchedFullName);
            }
          };
          fetchFullName();
        } catch (e) {
