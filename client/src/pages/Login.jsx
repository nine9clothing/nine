...
const Login = () => {
  ...
  // Enhanced session restoration and auth state listener
  useEffect(() => {
    let mounted = true;

    const restoreSession = async () => {
      try {
        setLoading(true); // Indicate session check is in progress
        const { data: { session }, error } = await supabase.auth.getSession();

        if (error) {
          console.error('Session retrieval error:', error.message);
          setToastMessage({ message: 'Error checking session. Please try again.', type: 'error' });
          return;
        }
        ...
        console.error('Unexpected error in session restore:', err.message);
        setToastMessage({ message: 'An unexpected error occurred.', type: 'error' });
      } finally {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        if (adminError) {
          console.error('Admin check error in listener:', adminError.message);
        }
