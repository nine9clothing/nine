...
const Checkout = () => {
  ...
  // State to track pending order after payment
  ...
  const restoreSession = useCallback(async () => {
    const { data: { session }, error } = await supabase.auth.getSession();
    if (error) {
      setToastMessage({ message: 'Failed to restore session. Please log in again.', type: 'error' });
      navigate('/login');
      return false;
    }
    if (!session) {
      setToastMessage({ message: 'No active session. Please log in.', type: 'error' });
      navigate('/login');
      return false;
    }
    return true;
  ...
  // Redirect to cart if no items or state
  useEffect(() => {
    if (!loading && cartItems.length === 0 && !location.state) {
      navigate('/cart');
    }
  ...
  // Fetch addresses after ensuring session
  useEffect(() => {
    const fetchAddresses = async () => {
  ...
  }, [user, loading, navigate, restoreSession]);
        ...
        handler: async function (response) {
          try {
            ...
            if (verifyResult.status === 'success') {
              ...
              // Restore session before completing order
              const sessionValid = await restoreSession();
              if (!sessionValid) {
  ...
  const completeOrder = async (paymentId = null, orderData = null) => {
    ...
    const data = orderData || {
      userId: user.id,
      cartItems,
      selectedAddress: addresses.find(addr => addr.id.toString() === selectedAddressId),
      selectedShippingOption,
      paymentMethod,
      subtotal,
      discount,
    ...
    try {
      ...
      const totalWeight = data.cartItems.reduce((weight, item) => weight + (0.35 * item.quantity), 0);
      ...
      const shiprocketPayload = {
        ...
        pickup_location: 'warehouse-1',
        billing: {
          customer_name: data.selectedAddress.name.split(' ')[0] || 'Customer',
          last_name: data.selectedAddress.name.split(' ').slice(1).join(' ') || '',
          address: data.selectedAddress.address,
          city: data.selectedAddress.city,
          pincode: data.selectedAddress.pincode,
          state: 'Maharashtra',
          country: 'India',
          email: user.email || 'customer@example.com',
        ...
        shipping: { is_billing: true },
        items: orderItems,
        sub_total: data.totalForDisplay,
        dimensions: { length: 16, breadth: 12, height: 16, weight: totalWeight },
        user_id: data.userId,
        shipping_charges: data.selectedShippingOption.rate,
        cod_fee: data.codFee,
        payment_method: data.paymentMethod === 'Cash on Delivery' ? 'COD' : 'Prepaid',
      ...
      const { data: orderData, error } = await supabase.from('orders').insert([{
        user_id: data.userId,
        order_id,
        total: data.totalForDisplay,
        total_amount: data.totalForDisplay,
        discount: data.discount + data.pointsDiscount,
        shipping_charges: data.selectedShippingOption.rate,
        cod_fee: data.codFee,
        items: orderItems,
        status: 'placed',
        shipping_name: data.selectedAddress.name,
        shipping_phone: data.selectedAddress.phone,
        shipping_address: data.selectedAddress.address,
        shipping_city: data.selectedAddress.city,
        shipping_pincode: data.selectedAddress.pincode,
        shipping_details: shiprocketResponse.data.shiprocket_response || shiprocketResponse.data,
        shipping_status: 'pending',
        display_order_id: order_id,
        courier_id: data.selectedShippingOption.courier_company_id,
        courier_name: data.selectedShippingOption.courier_name,
        estimated_delivery: data.selectedShippingOption.estimated_delivery_days,
      ...
      for (const item of orderItems) {
        ...
        try {
          ...
          if (currentStock < units) {
            console.warn(`Insufficient stock for SKU ${sku} (Size: ${selectedSize}). Current: ${currentStock}, Ordered: ${units}`);
            continue;
          }
      ...
      if (data.appliedPromo) {
        try {
          const promoId = data.appliedPromo.id;
          const { data: promoDetails, error: promoDetailsError } = await supabase
            .from('promocodes')
            .select('used')
            .eq('id', promoId)
          ...
          if (promoDetailsError) {
            console.error('Error fetching promo details:', promoDetailsError);
            throw promoDetailsError;
          }
          const currentOverallUsed = promoDetails.used;
          ...
          if (overallUpdateError) {
            console.error('Error updating promo used count:', overallUpdateError);
          }
          ...
          if (userUsageFetchError && userUsageFetchError.code !== 'PGRST116') {
            console.error('Error fetching user promo usage:', userUsageFetchError);
          } else if (userUsageData) {
            const { error: userUpdateError } = await supabase
              .from('promo_usage')
              .update({ usage_count: userUsageData.usage_count + 1 })
  ...
  const handleConfirmOrder = async () => {
    ...
    if (paymentInitiated) return; // Prevent multiple clicks
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={styles.container}>
        <div style={styles.mainColumn}>
          ...
          <button
            onClick={handleConfirmOrder}
            disabled={loadingOrder || !selectedAddressId || !selectedShippingOption || !paymentMethod || paymentInitiated}
            style={{
              ...styles.confirmOrderBtn,
              opacity: (!selectedAddressId || !selectedShippingOption || !paymentMethod || loadingOrder || paymentInitiated) ? 0.6 : 1
            }}
          >
            {loadingOrder ? 'Confirming...' : `Confirm Order - ₹${totalForDisplay.toFixed(2)}`}
          </button>
